...
db "f=mod(f,105.0);"
db "vec3 r=normalize(vec3(-2.0+0.01*gl_FragCoord.x,-5.0+0.01*gl_FragCoord.y,10.0));"
db "vec3 g=vec3(0,0,-10.0);"
db "float u=200;"
db "int c=0;"
db "while(u>0.1&&c<1000){"
db "vec3 p=g;"
db "u=-1.0*sin(p.x)*(sin(f)+1.2)+cos(f*20.0+p.z)+max(cos(p.y),sin(p.x))+2.0;"
db "r+=vec3(0.0,0.002,0.001);"
db "g=g+r*u;"
db "c++;"
db "}"
db "gl_FragColor.rgb=vec3(0.4+smoothstep(70.0,95.0,f),smoothstep(30.0,40.0,f)*smoothstep(90.0,75.0,f),0.8*smoothstep(70.0,50.0,f))*c/(75.0*3.0);"
db "}"
db 0 
