...
db "f/=10.0;"
db "p.x+=sin(f/20.0)*20.0;"
db "p.y+=cos(f/20.0);"
db "float a=(3.0+sin(p.x+1.5)+p.y+sin(p.z+f+f));"
db "float b=3.0+sin(p.x+1.5)+p.y+sin(p.z+f+f);"
db "float c=smoothstep(00.0, 100.0, f);"
db "return c*a+(1.0-c)*b;"
db "}"
db "void main() {"
db "vec3 eye=vec3(0.0,0.0,-10.0);"
db "vec3 a=vec3(0.5);"
db "vec3 l=vec3(5.5);"
db "vec3 li=vec3(0.7);"
db "vec3 mc=vec3(0.4,0.0,0.8);"
db "float f=dot(gl_Color.xyz,vec3(1,256,65536))*7.0;"
db "vec3 r=normalize(vec3(-4.0+0.0111*gl_FragCoord.x,-3.0+0.0125*gl_FragCoord.y,0)-eye );"
db "vec3 p=eye;"
db "float di=9.0;"
db "int c=0;"
db "vec3 df=vec3(0);"
...
db "if(di<0.1) {"
db "di=d(p,f);"
db "vec3 n=normalize(vec3("
db "d(p-vec3(e,0,0),f),"
db "d(p-vec3(0,e,0),f),"
db "d(p-vec3(0,0,e),f)"
db ")-vec3(di));"
db "df=li*dot(normalize(p-l),n);"
db "vec3 sd;"
db "sd=normalize(reflect(p-l,n));"
db "s=li*pow(clamp(-dot(sd,r),0.0,1.0),20.0);"
db "occ=0;"
db "for(int ao=1;ao<=5;ao++) {"
db "float aod=(ao*0.15);"
db "occ=occ+(1.0/pow(2.0,ao))*(aod-d(p-(n*aod),f));"
db "}"
db "occ=clamp(1.0-occ,0.0,1.0);"
db "break;"
db "}"
db "di=d(p,f);"
db "p=p+r*di;"
...
db "}"
db "}"
db "gl_FragColor.rgb=(a*mc*occ+mc*df+s)*occ+mc*(c/50.0);"
db "}"
db 0 
